...
```javascript
fetch('http://localhost:8000/api/tire/degradation/predict?compound=MEDIUM&race_laps=30&driver_code=VER')
  .then(res => res.json())
  .then(data => {
...
```javascript
fetch('http://localhost:8000/api/tire/strategy/recommend?race_distance=58&min_pit_stops=1&weather_condition=DRY', {
  method: 'POST'
})
  .then(res => res.json())
  .then(data => {
...
```javascript
fetch('http://localhost:8000/api/tire/pitstop/windows', {
  method: 'POST',
...
```javascript
fetch('http://localhost:8000/api/tire/realtime/insights?current_lap=25&current_compound=SOFT&stint_start_lap=15&driver_code=HAM')
  .then(res => res.json())
  .then(data => {
