...
const TireDegradation = () => {
  ...
  const fetchDegradationPrediction = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/tire/degradation/predict?compound=${compound}&race_laps=${raceLaps}`
      );
  ...
  const fetchStrategyRecommendation = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/tire/strategy/recommend?race_distance=${raceLaps}&min_pit_stops=1&weather_condition=DRY`,
        { method: 'POST' }
      );
      const data = await response.json();
