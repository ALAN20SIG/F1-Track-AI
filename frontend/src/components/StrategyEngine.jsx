...
const StrategyEngine = () => {
  ...
  const fetchDriversList = async () => {
    ...
    try {
      ...
      const response = await fetch('https://f1-track-ai-production.up.railway.app/api/db/drivers', {
      ...
      if (error.name === 'AbortError') {
        ...
        setError('Unable to load drivers. Please ensure backend is running on https://f1-track-ai-production.up.railway.app');
      }
    } finally {
      setLoadingDrivers(false);
    }
  ...
  const fetchStrategySuggestions = async () => {
    ...
    try {
      const url = `https://f1-track-ai-production.up.railway.app/api/analysis/strategy-suggestions/${selectedDriver}?target_position=${targetPosition}`;
      ...
      if (error.name === 'AbortError') {
        ...
        setError('Cannot connect to backend. Please ensure backend is running on https://f1-track-ai-production.up.railway.app');
      } else {
