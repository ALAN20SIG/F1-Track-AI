...
import requests
try:
    response = requests.get("https://f1-track-ai-backend.onrender.com/api/session/info", timeout=5)
...
for endpoint, name in endpoints:
    try:
        response = requests.get(f"https://f1-track-ai-backend.onrender.com{endpoint}", timeout=10)
        if response.status_code == 200:
            data = response.json()
            if data.get('success'):
...
print("  Backend API: https://f1-track-ai-backend.onrender.com/docs")
print("\nNew Components:")
